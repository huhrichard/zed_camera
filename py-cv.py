...
def main():
    ...
    while key != 113:
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            resize_width = int(new_width / 2)
            resize_height = int(new_height / 2)

            image_ocv = cv2.resize(image_ocv, (resize_width, resize_height))
            depth_image_ocv = cv2.resize(depth_image_ocv, (resize_width, resize_height))
    ...
    zed.close()
    fps.stop()
    print("[INFO] dimensions: " + str(new_width) + " x " + str(new_height))
